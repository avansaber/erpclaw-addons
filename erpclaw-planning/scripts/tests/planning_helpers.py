"""Shared helper functions for ERPClaw Planning unit tests.

Provides:
  - DB bootstrap via init_schema.init_db() + init_planning_schema()
...
ROOT_DIR = os.path.dirname(MODULE_DIR)            # erpclaw-planning/
...
def load_db_query():
    """Load erpclaw-planning db_query.py explicitly to avoid sys.path collisions."""
    db_query_path = os.path.join(MODULE_DIR, "db_query.py")
    spec = importlib.util.spec_from_file_location("db_query_planning", db_query_path)
...
def init_all_tables(db_path: str):
    """Create all ERPClaw core tables + planning vertical tables."""
    ...
    # 2. Planning vertical schema (4 tables)
    spec2 = importlib.util.spec_from_file_location("planning_init", VERTICAL_INIT_PATH)
    m2 = importlib.util.module_from_spec(spec2)
    spec2.loader.exec_module(m2)
    m2.init_planning_schema(db_path)
...
def seed_company(conn, name="Test Planning Co", abbr="TPC") -> str:
...
def seed_naming_series(conn, company_id: str):
    """Seed naming series for planning entity types."""
    series = [
        ("planning_scenario", "SCEN-", 0),
        ("planning_scenario_line", "SCNL-", 0),
        ("forecast", "FCST-", 0),
        ("forecast_line", "FSTL-", 0),
    ]
...
def build_env(conn) -> dict:
    """Create a full planning test environment.
