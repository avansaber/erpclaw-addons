"""Shared helper functions for ERPClaw CRM unit tests.
...
MODULE_DIR = os.path.dirname(TESTS_DIR)                    # erpclaw-crm/
...
def load_db_query():
    """Load erpclaw-crm db_query.py explicitly to avoid sys.path collisions."""
    db_query_path = os.path.join(MODULE_DIR, "db_query.py")
    spec = importlib.util.spec_from_file_location("db_query_crm", db_query_path)
...
def init_all_tables(db_path: str):
    ...
    # Foundation tables (company, naming_series, lead, opportunity, etc.)
    spec = importlib.util.spec_from_file_location("init_schema", INIT_SCHEMA_PATH)
    m = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(m)
    m.init_db(db_path)

    # Growth vertical tables (24 tables: CRM adv + AI engine)
...
def seed_company(conn, name="Test CRM Co", abbr="TCC") -> str:
...
def seed_naming_series(conn, company_id: str):
    """Seed naming series for CRM entity types."""
    series = [
        ("lead", "LEAD-", 0),
        ("opportunity", "OPP-", 0),
        ("campaign", "CAMP-", 0),
...
def build_env(conn) -> dict:
    """Create a full CRM test environment.

    Returns dict with all IDs needed for CRM domain tests.
    """
