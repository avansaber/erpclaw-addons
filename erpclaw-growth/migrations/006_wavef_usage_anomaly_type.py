"""erpclaw-growth migration 006: extend anomaly.anomaly_type CHECK (Wave F AI1).

Adds ONE Wave F anomaly type to the growth-owned `anomaly` table's
anomaly_type CHECK so existing installs accept what the new usage detector emits:

  - rate_plan_mismatch — meter usage exceeds what the customer's rate plan
    allows/defines (prepaid balance exhausted/overage, or consumption beyond
    a quantity-banded plan's top tier ceiling).

The other half of the Wave F usage pair (consumption spike > N x the meter's
historical baseline) emits the PRE-DECLARED `consumption_spike` type, which has
been in the CHECK since the original 16-type enum — no migration needed for it.
...
converge. Namespaced 'erpclaw-growth:006' in the shared erpclaw_schema_migration
ledger via the P1 module-migration runner (module_manager -> migration_runner).

Dialect-aware + idempotent, mirroring migration 005 (the Wave 2 CHECK-extension;
foundation 024_displace_opportunity_stage_check.py is the structural precedent):
  ...
  - SQLite cannot ALTER a CHECK, so the table is rebuilt: rename -> recreate WITH
    ...
    columns and NO inbound FK references (verified at 005), so no FK-off rebuild
    dance is needed. Idempotency guard: if the new value already appears in the
    stored table SQL, the rebuild is skipped.
...
Usage:
    python3 006_wavef_usage_anomaly_type.py [--db-path PATH]
"""
import argparse
import os
import sqlite3
...
_NEW_VALUE_PROBE = "rate_plan_mismatch"

# The `anomaly` table WITH the extended CHECK (20 existing + 1 Wave F). Authored
...
def _run_sqlite(path):
    ...
    try:
        conn.execute("BEGIN")
        conn.execute("ALTER TABLE anomaly RENAME TO anomaly_wfai1_old")
        conn.execute(_ANOMALY_DDL_EXTENDED)
        conn.execute(_ANOMALY_COPY_SQL)
        conn.execute("DROP TABLE anomaly_wfai1_old")
    ...
    dangling = conn.execute(
        "SELECT name FROM sqlite_master WHERE type='table' AND name LIKE '%_wfai1_old'"
    ).fetchall()
    if dangling:
        conn.close()
        raise RuntimeError(f"Migration 006 left dangling temp table: {[r[0] for r in dangling]}")
    print(f"  anomaly.anomaly_type CHECK extended (+1 Wave F type); {n} rows preserved.")
    conn.close()
...
def _run_postgres(url):
    ...
    try:
        ...
        conn.commit()
        print("  Postgres: anomaly.anomaly_type CHECK extended (+1 Wave F type).")
    finally:
        conn.close()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="erpclaw-growth migration 006: extend anomaly.anomaly_type CHECK (Wave F AI1)")
    parser.add_argument("--db-path", default=DEFAULT_DB_PATH)
    args = parser.parse_args()
    run_migration(args.db_path)
    print("erpclaw-growth migration 006 complete.")
