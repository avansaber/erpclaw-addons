"""Shared helper functions for ERPClaw Logistics unit tests.

Provides:
  - DB bootstrap via init_schema.init_db() + create_logistics_tables()
  - call_action() / ns() / is_error() / is_ok()
  - Seed functions for company, supplier, naming series
...
MODULE_DIR = os.path.dirname(TESTS_DIR)          # erpclaw-logistics/scripts/
ROOT_DIR = os.path.dirname(MODULE_DIR)            # erpclaw-logistics/
...
def load_db_query():
    """Load erpclaw-logistics db_query.py explicitly to avoid sys.path collisions."""
    db_query_path = os.path.join(MODULE_DIR, "db_query.py")
    spec = importlib.util.spec_from_file_location("db_query_logistics", db_query_path)
...
def init_all_tables(db_path: str):
    """Create all ERPClaw core tables + logistics vertical tables."""
    ...
    # 2. Logistics vertical schema (8 tables)
    spec2 = importlib.util.spec_from_file_location("logistics_init", VERTICAL_INIT_PATH)
    m2 = importlib.util.module_from_spec(spec2)
    spec2.loader.exec_module(m2)
    m2.create_logistics_tables(db_path)
...
def seed_company(conn, name="Test Logistics Co", abbr="TLC") -> str:
...
def seed_supplier(conn, company_id: str, name="Test Carrier Supplier") -> str:
...
def seed_naming_series(conn, company_id: str):
    """Seed naming series for logistics entity types."""
    series = [
        ("logistics_carrier", "CAR-", 0),
        ("logistics_shipment", "SHIP-", 0),
        ("logistics_route", "RTE-", 0),
        ("logistics_carrier_invoice", "CINV-", 0),
    ]
...
def build_env(conn) -> dict:
    """Create a full logistics test environment.

    Returns dict with company_id, supplier_id, and all naming series seeded.
    """
    cid = seed_company(conn)
    seed_naming_series(conn, cid)
    sup = seed_supplier(conn, cid, "FastFreight Inc")
    return {
        "company_id": cid,
        "supplier_id": sup,
    }
