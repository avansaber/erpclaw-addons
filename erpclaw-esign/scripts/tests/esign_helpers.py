"""Shared helper functions for ERPClaw E-Sign unit tests.

Provides:
  - DB bootstrap via init_schema.init_db() + init_esign_schema()
...
ROOT_DIR = os.path.dirname(MODULE_DIR)                     # erpclaw-esign/
...
# Only when erpclaw_lib is not already reachable. An unconditional insert at
# position 0 overrides a caller that deliberately bound a different tree — which
# is how this suite imported the DEPLOYED lib instead of the one under test and
# could not find `erpclaw_lib.seam` after the ADR-0034 conversion.
...
if importlib.util.find_spec("erpclaw_lib") is None and ERPCLAW_LIB not in sys.path:
...
def load_db_query():
    """Load erpclaw-esign db_query.py explicitly to avoid sys.path collisions."""
    db_query_path = os.path.join(MODULE_DIR, "db_query.py")
    spec = importlib.util.spec_from_file_location("db_query_esign", db_query_path)
...
def init_all_tables(db_path: str):
    """Create all foundation + esign tables."""
    ...
    # E-Sign vertical tables (2 tables)
    spec2 = importlib.util.spec_from_file_location("esign_init", VERTICAL_INIT_PATH)
    m2 = importlib.util.module_from_spec(spec2)
    spec2.loader.exec_module(m2)
    m2.init_esign_schema(db_path)
...
def seed_company(conn, name="Test ESign Co", abbr="TES") -> str:
...
def seed_naming_series(conn, company_id: str):
    """Seed naming series for E-Sign entity types."""
    series = [
        ("signature_request", "ESIG-", 0),
    ]
...
def build_env(conn) -> dict:
    """Create a full esign test environment.

    Returns dict with all IDs needed for esign domain tests.
    """
